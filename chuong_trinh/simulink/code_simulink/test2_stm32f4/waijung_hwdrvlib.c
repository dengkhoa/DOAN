...
uint32_t profiler_records[profilerPackageSize];
uint32_t *profiler_records_ptr;
DMA_InitTypeDef TargetSetup_DMA_Init;
...
/* Data read structure */
UARTRX_BUFFER_READ_STRUCT UARTRx_read_structure = { 0, 0, (uint8_t *)0 };

uint8_t UARTRx_Receive(uint8_t *buffer, uint16_t size)
{
  const char header[1] = { 0 };        /* Header */

  const char terminator[1] = { 0 };

  return ( UART2_ReadBinary(&UARTRx_read_structure, header, 1, terminator, 0,
           buffer, size));
}
...
void enable_BasicPWM(void)
{
  ...
  TIM_TimeBaseStructure.TIM_Period = BasicPWM_TIM4_ARR-1;/* 5.0E-5 sec */
...
void profiler_init(void)
{
  ...
  /* Initialize profiler data packet */
  profiler_records[0] = profilerHeader;
  profiler_records_ptr = &profiler_records[profilerAdditionalRecordLength];
  ...
  /* DMA init struct configuration */
  TargetSetup_DMA_Init.DMA_PeripheralBaseAddr = (uint32_t)(&USART3->DR);
  TargetSetup_DMA_Init.DMA_PeripheralInc = DMA_PeripheralInc_Disable;
  TargetSetup_DMA_Init.DMA_MemoryInc = DMA_MemoryInc_Enable;
  TargetSetup_DMA_Init.DMA_PeripheralDataSize = DMA_PeripheralDataSize_Byte;
  TargetSetup_DMA_Init.DMA_MemoryDataSize = DMA_MemoryDataSize_Byte;
  TargetSetup_DMA_Init.DMA_FIFOMode = DMA_FIFOMode_Disable;
  TargetSetup_DMA_Init.DMA_FIFOThreshold = DMA_FIFOThreshold_Full;
  TargetSetup_DMA_Init.DMA_MemoryBurst = DMA_MemoryBurst_Single;
  TargetSetup_DMA_Init.DMA_PeripheralBurst = DMA_PeripheralBurst_Single;

  /* Configure DMA controller to manage TX DMA request */
  TargetSetup_DMA_Init.DMA_Memory0BaseAddr = (uint32_t)profiler_records;
  TargetSetup_DMA_Init.DMA_Channel = DMA_Channel_4;
  TargetSetup_DMA_Init.DMA_DIR = DMA_DIR_MemoryToPeripheral;
  TargetSetup_DMA_Init.DMA_BufferSize = (uint16_t) sizeof profiler_records;
  TargetSetup_DMA_Init.DMA_Mode = DMA_Mode_Normal;
  TargetSetup_DMA_Init.DMA_Priority = DMA_Priority_High;
}
...
void profiler_update(void)
{
  profiler_records_ptr = &profiler_records[profilerAdditionalRecordLength];
  CRC_ResetDR();
  profiler_records[profilerCRCLength] = CRC_CalcBlockCRC((uint32_t *)
    profiler_records, profilerCRCLength);

  /* Write to DMA buffer */
  /* Update overflow counter */
  /* Setup */
  DMA_Init(DMA1_Stream3, &TargetSetup_DMA_Init);
